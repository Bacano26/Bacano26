...
import { Minus, Plus, Ticket, ShoppingCart, CheckCircle } from 'lucide-react'
...
interface Props {
  evento: Evento
  userId: string  // viene del servidor, ya sabemos que está logueado
}
...
export default function FormCompra({ evento, userId }: Props) {
  ...
  const maximo = Math.min(10, boletasRestantes)  // máximo 10 por compra
  ...
  // Genera un código único para cada boleta
  ...
  async function handleComprar() {
    ...
    try {
      ...
      // PASO 2: Crear una boleta individual por cada unidad comprada
      ...
      if (errorBoletas) {
        setError('Compra creada pero error generando boletas. Contacta soporte.')
        return
      }

      // PASO 3: Actualizar boletas_vendidas en el evento
      ...
      if (errorEvento) {
        console.error('Error actualizando disponibilidad:', errorEvento)
        // No bloqueamos al usuario, la compra ya fue exitosa
      }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            className="w-full py-3 bg-gray-50 hover:bg-gray-100 text-gray-700 font-semibold rounded-xl transition-colors text-sm"
          >
            Ver más eventos
          </button>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={disminuir}
            disabled={cantidad <= 1}
            className="w-9 h-9 rounded-xl border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
      ...
      <button
        ...
        className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-semibold rounded-xl transition-colors text-sm flex items-center justify-center gap-2 disabled:cursor-not-allowed"
      >
        {cargando ? (
          <>
